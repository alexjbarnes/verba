...
// GPL-free build: piper_phonemize and espeak-ng (both GPL) are removed so the
// final binary links none of their code. They are only pulled in by sherpa's
// TTS (SHERPA_ONNX_ENABLE_TTS); the Android sherpa native build is configured
// with -DSHERPA_ONNX_ENABLE_TTS=OFF (see scripts/android-build.sh) so neither
// the archives nor any symbol references exist. TTS is provided by the Piper
// `ort` path in src/piper.rs instead.
//
// `ucd` (Unicode Character Database) is also removed: it is only a dependency of
// piper_phonemize/espeak-ng, so the TTS-off sherpa build no longer produces
// libucd.a — keeping it here would emit `-lucd` for a nonexistent archive and
// fail the link.
