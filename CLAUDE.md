...
Third top-level mode beside Speak/Listen — a local meeting assistant. Live-transcribes the microphone (you) AND system-audio loopback (everyone else) with the same Parakeet model, then summarizes with a local LLM. The live transcript stores no audio; at stop the loopback speech is briefly buffered in RAM to reconstruct the timeline for an offline speaker-diarization pass, then discarded. Voiceprints (speaker embeddings, not audio) persist so a person named once is recognized across later meetings.

- `mod.rs` - `MeetingSession` singleton: two `AudioRecorder`s (mic `ConfigInput` + resolved loopback), one consumer thread per stream feeding the engine's SHARED transcriber (`engine::transcriber_arc()` — Parakeet loads once, segments interleave through its worker mutex). Utterances tagged with segment-start wall-clock + speaker; streamed as `meeting-utterance` events (text stripped of disfluencies via `postprocess::remove_fillers` — no grammar rewrite, so meetings stay verbatim); transcript autosaved every 30s (the autosaver watches the session's own `stopping` flag, NOT `MEETING_ACTIVE` — a new meeting can re-raise the global while the old finalize thread still runs). Interlocks with dictation both ways via `MEETING_ACTIVE` + `AppState.recording` (single mic). `summarize(id)` reads the stored transcript back and map-reduces it. `stop()` only ends capture and returns provisional meta with `MeetingMeta.processing = true`; a detached finalize thread drains consumers, transcribes tails, runs `refine_speakers()`, writes the transcript, clears the flag, and emits `meeting-state {state:"processed", id}` — so a new meeting can start while the old one processes (the meetings-list card shows Processing and blocks open/delete; stale flags from a killed process are cleared at store load). `refine_speakers()` reconstructs the loopback waveform from buffered segments, runs the offline diarizer, and relabels the system utterances (gallery-matched name or `Speaker N`); `rename_speaker(id, from, to)` enrolls a speaker's voiceprint under a new name and rewrites the transcript; `transcript(id)` returns structured lines for the per-speaker filter view (click a speaker to see only their lines); `meeting_gallery_*` commands manage the cross-meeting known-speakers list in Settings
- `loopback.rs` - per-OS system-audio resolution (Windows = default output as input; macOS 14.6+ Core Audio tap gated on `kern.osproductversion`; Linux `.monitor` source). `Unsupported(reason)` -> mic-only with a UI notice. The three mechanisms select along DIFFERENT axes, which is what `sources()` reports: `apps` (macOS, bundle ids), `outputs` (Windows, device names), `none` (Linux). `config.meeting_output_device` stores whichever id kind applies, and the frontend renders the list the backend reports rather than branching on the user agent
- `system_tap.rs` (macOS) - process tap, either all apps or one bundle id (`start_tap`). Bundle ids resolve to live process objects at tap creation because those ids are reassigned per launch; an app that isn't running falls back to capturing everything rather than recording silence. `list_audio_apps()` enumerates `kAudioHardwarePropertyProcessObjectList`, keeps entries with a bundle id, and names them from the `.app` in `proc_pidpath`. A DEVICE-bound tap was tried and rejected: it silently delivers silence for some outputs, notably Bluetooth
- `set_device(app, kind, name)` - swap a LIVE meeting's mic or system source: builds the new recorder before stopping the old, flushes the old stream's tail through the transcriber (detached), spawns a fresh consumer on the same `started_at`, restores the previous stream if the new device won't open, and persists the name to config. A swapped system stream restarts its live speaker numbering (the offline pass relabels at stop). Both Settings and the live view drive it through the same frontend path
...
- `store.rs` - `meetings.json` index + Markdown transcript/summary writers to the configured dirs. `search(query)` is full-text across every meeting (speaker names + spoken words from the sidecar, markdown fallback for pre-sidecar ones, title/tags as a last resort), returning one snippet + match count per meeting; the frontend debounces it and groups results by day/week. `MeetingMeta.tags` are free-form labels doubling as the grouping axis: `seed_series_tag(id)` runs after finalize and auto-applies a `series_slug(title)` tag when a title recurs (backfilling the one older meeting only when the series first forms, so a removed tag stays removed); a one-time `backfill_series_tags` sweep at store load tags the pre-existing archive, guarded by a `.series-backfilled` sentinel beside `meetings.json`
...
Three files: `index.html`, `main.js`, `styles.css`. No build step, no framework. Uses Tailwind + Material Symbols icons + the Inter font, all VENDORED into `src/` (`tailwind.js`, `src/fonts/*.woff2`), never from a CDN: the Tailwind Play script generates every utility class at runtime, so a cold/auto-launch before the network is up rendered the app white; the fonts failed the same way (icon names shown as literal words). Local assets served from `frontendDist` (`../src`) can't fail that way. The inline `tailwind.config` still applies (the Play engine reads it), and the `.material-symbols-outlined` class rules (esp. the `'liga'` feature) live in `styles.css` because Google's CSS used to provide them. Communicates with Rust via `window.__TAURI__.core.invoke()` and `window.__TAURI__.event.listen()`. Tauri embeds these at compile time via `generate_context!()`.
