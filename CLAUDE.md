...
- `loopback.rs` - per-OS system-audio device resolution (Windows = default output as input; macOS 14.6+ Core Audio tap gated on `kern.osproductversion`; Linux `.monitor` source). `Unsupported(reason)` -> mic-only with a UI notice. NOTE the `meeting_output_device` preference only changes capture on Windows — macOS taps every process globally and Linux always takes the default monitor, so both ignore the name
